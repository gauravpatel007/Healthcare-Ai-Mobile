...
clean_twiml = re.sub(r'([a-zA-Z_:][a-zA-Z0-9._:-]*)\s*=\s*"([^"]*)"', fix_attr, twiml_val)
root = ET.fromstring(clean_twiml)
print('SUCCESS! Root tag:', root.tag)
print('Children:', [n.tag for n in root])
action = root.find('Gather').get('action')
print('Gather action:', action)
