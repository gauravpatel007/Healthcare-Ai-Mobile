...
export default function AdminFileManager() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500 pb-1">
      <div className="bg-white dark:bg-gray-800 rounded-2xl sm:rounded-[2rem] p-4 sm:p-5 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between gap-3 w-full shrink-0">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 sm:gap-3">
              {activeFolder && (
                <button onClick={() => setActiveFolder(null)} className="p-1.5 sm:p-2 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg sm:rounded-xl transition-colors shrink-0">
        ...
        <button 
          ...
          disabled={uploading}
          title="Upload File"
          className="w-11 h-11 sm:w-12 sm:h-12 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-2xl flex items-center justify-center shadow-md transition-all active:scale-95 shrink-0"
        >
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-3.5 sm:p-6">
          {filteredFiles.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
              ...
              <p className="text-gray-400 font-medium text-sm mb-4">Upload a file to get started.</p>
              <button 
                onClick={() => fileInputRef.current?.click()}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl flex items-center gap-2 shadow-sm transition-all"
              >
            ...
            <>
              {/* Mobile Card List View (Ensures all buttons: Edit, Download, Delete are 100% visible) */}
              <div className="sm:hidden space-y-2.5">
                {filteredFiles.slice(0, displayCount).map(file => (
                  <div 
                    key={file.id} 
                    className="p-3 bg-gray-50/80 dark:bg-gray-700/30 rounded-2xl border border-gray-100 dark:border-gray-700/60 flex items-center justify-between gap-2.5 transition-colors"
                  >
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center shrink-0 shadow-inner">
                        <FileText className="w-5 h-5" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="font-bold text-sm text-gray-900 dark:text-white truncate" title={file.name}>
                          {file.name}
                        </p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                          {formatSize(file.size_bytes)} • <span className="uppercase text-[10px] font-bold text-blue-600 dark:text-blue-400">{file.type?.split('/')[1] || file.type}</span>
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <button 
                        ...
                        className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 rounded-lg transition-colors"
                        title="Edit"
                      >
                      ...
                      <button 
                        ...
                        className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors"
                        title="Download"
                      >
                      ...
                      <button 
                        ...
                        className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors"
                        title="Delete"
                      >
              ...
              <div className="hidden sm:block overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[500px]">
                  ...
                  <tbody>
                    {filteredFiles.slice(0, displayCount).map(file => (
                      <tr key={file.id} className="border-b border-gray-50 hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors">
          ...
          {filteredFiles.length > displayCount && (
            <div className="pt-4 pb-1 mt-3 flex justify-center w-full">
              <button
                onClick={() => setDisplayCount(prev => prev + 20)}
                className="px-5 py-2.5 text-xs sm:text-sm font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-xl transition-all shadow-sm active:scale-95 flex items-center gap-2 border border-blue-100 dark:border-blue-800/40"
              >
                <span>See More ({filteredFiles.length - displayCount} more)</span>
              </button>
            </div>
          )}
        </div>
